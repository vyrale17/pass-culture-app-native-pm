...
1. **Si l'offre a un produit associé** → Retourner les artistes du produit (via `ArtistProductLink`)
2. **Si l'offre n'a pas de produit** → Retourner les artistes liés directement à l'offre (via `ArtistOfferLink`)
...
**Champ ajouté** : `artists` (liste d'objets)

### Structure de chaque artiste

| **Champ** | **Type** | **Description** | **Exemple** |
| --- | --- | --- | --- |
| `id` | `string` ou `null` | Identifiant de l'artiste. `null` si artiste custom (non référencé).<br>**Frontend utilise `id != null` pour la cliquabilité** | `"456"` ou `null` |
| `name` | `string` | Nom de l'artiste | `"Katy Perry"` |
| `image` | `string` ou `null` | URL de la photo de l'artiste. `null` si custom ou pas de photo | `"https://storage.passculture.app/..."` |
| `role` | `string` | Rôle de l'artiste (enum) | `"PERFORMER"`, `"STAGE_DIRECTOR"`, etc. |

### Exemples de réponses API

**Exemple 1 : Offre avec produit (film "Parasite")**
...
**Exemple 2 : Offre sans produit (avant-première avec invité)**
...
Les artistes avec le champ `is_blacklisted = true` sont **automatiquement exclus** de la réponse API.

**Où ?** Côté backend, avant de retourner la liste au frontend.

**Raison** : Protection contre l'affichage d'artistes controversés ou interdits.
...
| **1 artiste custom** (`id == null`) | "de/Avec [Artiste]" | Non | "de Intervenant local" |
...
| **Musique Live**<br>*(2 cas)* | **Cas 1 :** "Avec [Interprètes]" 〉<br><br>**Cas 2 :** "de [Auteur]" 〉<br>"Avec [Interprètes]" 〉 | 1 champ<br><br>2 champs | "Avec Katy Perry, Gims et 2 autres" 〉<br><br>"de Bong Joon Ho" 〉<br>"Avec Robert Pattinson, Anamaria... et 2 autres" 〉 |
...
| **Festivals**<br>*(2 cas)* | **Cas 1 :** "Avec [Interprètes]" 〉<br>**Cas 2 :** "de [Auteur]" 〉<br>"Avec [Interprètes]" 〉 | 1 champ<br><br>2 champs | "Avec Katy Perry, Gims et 2 autres" 〉<br><br>"de Bong Joon Ho" 〉<br>"Avec Robert Pattinson, Anamaria... et 2 autres" 〉 |
...
| --- | --- | --- |
| **1 seul artiste référencé** (`id != null`) | Navigation directe vers la page artiste | Pas d'ambiguïté, gain de temps pour l'utilisateur |
| **1 seul artiste custom** (`id == null`) | Pas de navigation | Pas de page artiste disponible |
...
- Pas de clic possible → Pas de navigation car pas de page artiste
...
- Si `id == null` → Artiste custom → Pas de page disponible (non cliquable)
...
**Condition** : Clic sur une ligne affichant **2+ artistes**
...
- [x] Cliquabilité basée sur `id != null` (pas de champ supplémentaire)
...
**Version** : 1.2
**Date** : Décembre 2024
**Statut** : ✅ Validé avec règle d'exclusion mutuelle et cliquabilité basée sur `id != null`
